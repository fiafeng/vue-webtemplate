...
export default {
  ...
  faceReg(content) {
    content = content.replace(/\[[^\[^\]]+\\]/g, (word) => {
      ...
      if (index > -1) {
        let url =  "/static/emoji/q" + (index + 1) + ".gif";
        return '<img loading="lazy" style="vertical-align: middle;width: 32px;height: 32px" src="' + url + '" title="' + word + '" alt=""/>';
      } else {
        return word;
      }
    });
  ...
  pictureReg(content) {
    content = content.replace(/\[[^\[^\]]+\\]/g, (word) => {
      ...
      if (index > -1) {
        ...
        return '<img loading="lazy" class="pictureReg" style="border-radius: 5px;width: 100%;max-width: 250px;display: block" src="' + arr[1] + '" title="' + arr[0] + '" alt=""/>';
      } else {
        return word;
      }
    });
