...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saveLocal(fd) {
        upload(constant.baseURL + "/resource/upload", fd)
          .then((res) => {
      ...
      saveQiniu(fd) {
        request.get(constant.baseURL + "/qiniu/getUpToken", {key: fd.get("key")})
          .then((res) => {
            if (!common.isEmpty(res.data)) {
              fd.append("token", res.data);

              request.uploadQiniu(this.$store.state.sysConfig.qiniuUrl, fd)
                .then((res) => {
                  if (!common.isEmpty(res.key)) {
                    this.clearContext();
                    let url = this.$store.state.sysConfig['qiniu.downloadUrl'] + res.key;
                    let file = fd.get("file");
                    common.saveResource(this, "graffiti", url, file.size, file.type, null, "qiniu");
                    let img = "[你画我猜," + url + "]";
                    this.$emit("addGraffitiComment", img);
                  }
                })
