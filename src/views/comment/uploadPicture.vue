  ...
  export default {
    ...
    methods: {
      ...
      customUpload(options) {
        ...
        let key = this.prefix + "/" + (!common.isEmpty(this.username) ? (this.$store.state.currentUser.username.replace(/[^a-zA-Z]/g, '') + this.$store.state.currentUser.id) : (this.$store.state.currentAdmin.username.replace(/[^a-zA-Z]/g, '') + this.$store.state.currentAdmin.id)) + new Date().getTime() + Math.floor(Math.random() * 1000) + suffix;
        ...
        if (this.storeType === "local") {
          ...
          return request.upload(constant.baseURL + "/resource/upload", fd, this.isAdmin, options);
      ...
      handleSuccess(response, file, fileList) {
        ...
        if (this.storeType === "local") {
          ...
          common.saveResource(this, this.prefix, url, file.size, file.raw.type, file.name, "qiniu", this.isAdmin);
        }
